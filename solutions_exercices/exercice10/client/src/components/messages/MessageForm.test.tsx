...
test("les données entrées par l'utilisateur sont bien envoyées lors de la soumission", async () => {
  // ARRANGE
  let requeteBody = new FormData();
  let requeteParams: PathParams = {};

  serveur.use(
    http.post(`${apiUrl}/api/groupes/:groupeId/messages`, async ({params, request}) => {
      requeteParams = params;
      requeteBody = await request.formData();
  ...
  expect(requeteBody.get("texte")).toEqual("Un message de test");
  expect(requeteBody.get("nomUtilisateur")).toEqual("Utilisateur de test");
  expect(requeteBody.get("fichiers")).toEqual(null);
