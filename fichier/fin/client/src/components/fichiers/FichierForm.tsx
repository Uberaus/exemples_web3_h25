...
export function FichierForm() {
  ...
  useEffect(() => {
    let erreur = "";
    if (fichier && fichier.size > MAX_TAILLE_FICHIER) {
      ...
      >
        ...
        <Form.Group>
          ...
          <Form.Control
            ...
            accept="image/png, image/jpeg, image/jpg, .pdf, .docx, .xlsx, .txt"
            onChange={(event: ChangeEvent<HTMLInputElement>) =>
              setFichier(event.target.files ? event.target.files[0] : undefined)
            }
          />
